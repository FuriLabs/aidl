...
CXXFLAGS += -std=c++17 -O0 -g -Wno-error -Wno-everything
...
$(NAME).so: aidl_language_l.cpp aidl_language_y.cpp $(SOURCES)
	$(CXX) $(SOURCES) -o $(NAME).so.0 $(CXXFLAGS) $(CPPFLAGS) $(LDFLAGS)
	ln -sf $(NAME).so.0 $(NAME).so

clean:
	rm -f $(NAME).so.0 $(NAME).so aidl_language_l.cpp aidl_language_y.cpp aidl_language_y.h aidl_language_y.hpp
